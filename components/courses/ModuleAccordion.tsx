        ...
        {modules.map((module, index) => {
          ...
          return (
            <AccordionItem
              key={module._id}
              value={module._id}
              className="border border-zinc-800 rounded-xl overflow-hidden bg-zinc-900/70 data-[state=open]:bg-zinc-900/90"
            >
              <AccordionTrigger className="px-5 py-4 hover:no-underline hover:bg-zinc-800/50 transition-colors">
                <div className="flex items-center gap-4 flex-1">
                  <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-white text-violet-600 text-sm font-bold shrink-0">
                    {index + 1}
                  </div>
                  ...
                  {userId && total > 0 && (
                    <div className="hidden sm:flex items-center gap-3 shrink-0 w-36">
                      <Progress
                        ...
                        className="flex-1 h-2 bg-white [&>div]:bg-emerald-400"
                      />
                      <div className="w-5">
                        {isModuleComplete && (
              ...
              <AccordionContent className="px-5 pb-4 pt-2">
                <div className="ml-4 border-l-2 border-zinc-200 pl-3 space-y-1">
                  {module.lessons?.map((lesson, lessonIndex) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {hasVideo && (
                          <Play className="w-4 h-4 text-emerald-400 group-hover:text-violet-400 transition-colors" />
                        )}
                      </Link>
                    );
                  })}
