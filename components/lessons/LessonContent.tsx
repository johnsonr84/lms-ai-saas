...
const components: PortableTextComponents = {
  block: {
    ...
    h2: ({ children }) => (
      <h2 className="text-2xl font-bold mt-6 mb-3 text-black">{children}</h2>
    ),
    h3: ({ children }) => (
    ...
    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-violet-500 pl-4 my-4 italic text-violet-700">
        {children}
      </blockquote>
    ),
  },
  list: {
    bullet: ({ children }) => (
      <ul className="list-disc list-inside space-y-2 mb-4 text-fuchsia-600">
        {children}
      </ul>
    ),
    number: ({ children }) => (
  ...
  marks: {
    strong: ({ children }) => (
      <strong className="font-semibold text-violet-500">{children}</strong>
    ),
    em: ({ children }) => <em className="italic">{children}</em>,
    code: ({ children }) => (
      <code className="bg-white px-1.5 py-0.5 rounded text-sm text-violet-500 font-mono">
        {children}
      </code>
    ),
    link: ({ children, value }) => (
